/**************************************************************************
 * GroupListActivity.java, demactor Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Group entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Group list. A click on an item will get 
 * you to GroupShowActivity.
 * - On a tablet, the GroupShowFragment will be displayed right next to
 * the list.
 */
public class GroupListActivity 
    ...
    protected GroupListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected GroupShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (GroupShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (GroupListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, GroupShowActivity.class);
            final Group item = GroupContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(GroupContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Group item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Group item = GroupContract.cursorToItem(
