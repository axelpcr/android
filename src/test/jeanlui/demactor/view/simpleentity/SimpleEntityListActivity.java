/**************************************************************************
 * SimpleEntityListActivity.java, demactor Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display SimpleEntity entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a SimpleEntity list. A click on an item will get 
 * you to SimpleEntityShowActivity.
 * - On a tablet, the SimpleEntityShowFragment will be displayed right next to
 * the list.
 */
public class SimpleEntityListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected SimpleEntityListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected SimpleEntityShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (SimpleEntityShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (SimpleEntityListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_simpleentity_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, SimpleEntityShowActivity.class);
            final SimpleEntity item = SimpleEntityContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(SimpleEntityContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(SimpleEntity item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            SimpleEntity item = SimpleEntityContract.cursorToItem(
