/**************************************************************************
 * GroupToCommentListActivity.java, demactor Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display GroupToComment entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a GroupToComment list. A click on an item will get 
 * you to GroupToCommentShowActivity.
 * - On a tablet, the GroupToCommentShowFragment will be displayed right next to
 * the list.
 */
public class GroupToCommentListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected GroupToCommentListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected GroupToCommentShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (GroupToCommentShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (GroupToCommentListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_grouptocomment_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, GroupToCommentShowActivity.class);
            final GroupToComment item = GroupToCommentContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(GroupToCommentContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(GroupToComment item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            GroupToComment item = GroupToCommentContract.cursorToItem(
